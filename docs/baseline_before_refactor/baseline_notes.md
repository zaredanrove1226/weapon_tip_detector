...
- stable: ON
- profile: spear_body
- candidate mask mode: depth_candidate_or_dark
- best: accepted
- score: 约 0.77 / 0.300
...
- stable: ON
- profile: fist_body
- candidate mask mode: depth_candidate_and_dark
- best: accepted
- score: 约 0.66 / 0.300
...
palm 的检测链路可用，但固定视角下更依赖 ROI 和 depth candidate mask。
...
- cond: 0
- best: none
- 现象：固定视角下虽然 dark 有像素，但 depth_candidate 为 0，depth_candidate_and_dark 导致 candidate 为 0。
...
- cond: 0
- best: none
- 现象：固定视角下 palm 主体在 ROI 下方，ROI 偏上；同时 depth_cand=0，depth_candidate_and_dark 导致 candidate 为 0。
