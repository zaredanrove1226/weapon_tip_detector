  ...
  const ProfileBundle & profiles)
{
  ...
  result.slot_base_depth = estimateSlotBaseDepth(
    color_depth,
    display_roi,
  ...
  if (tip_type == "fist" && config_.enable_fist_dual_profile) {
    ...
    // Palm is handled by PalmReferenceDetector in the node.
    // This fallback keeps the pipeline valid when palm_reference_enable=false,
    // but no longer uses the old palm core / expanded strategy.
    ...
    result.active_profile = main_profile;
    result.active_profile_name = "palm";
...
double DetectionPipeline::estimateSlotBaseDepth(
  const cv::Mat & color_depth,
  const cv::Rect & roi,
